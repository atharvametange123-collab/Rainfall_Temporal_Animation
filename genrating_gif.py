...
def update_map(frame_idx):
  ...
  clean_date = pd.to_date_time(raw_date).strftime('%B %d')
  im = daily_slice.plot(ax=ax, cmap = "Reds", add_colorbar=False, vmin= 0, vmax= 50 )
  ax.set_title(f"Rainfall Timeline: {clean_date}")
  ax.axis('off')
  return[im]
  spatial_animation = animation.FuncAniamtion(fig, update_map, frames= 365, interval = 100)
  spatial_animation.save("WOOOOO_animation.gif, writer = 'FFmpeg')
